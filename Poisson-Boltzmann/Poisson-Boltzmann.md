...
When we discussed our variational approach and mean field theory, we assumed that the two-body potential energy decays sufficiently rapidly, $U\propto r^{-\alpha}$, with $\alpha > D$. But what happens if the potential decays more slowly, like the eletrostatic potential $U= e^2/(4\pi \epsilon_0 r )$? Then,
 
$$
\langle H\rangle_N\propto \int d^3r U(r)=4\pi \int_{r_0}^\infty r^2 U(r) dr = e^2 \int_{r_0}^\infty r dr=\infty\;.
$$

Thus, the interactions are too strong to apply our previous mean field /variational approaches, which assumed we can approximate the true gas by a modified version of the ideal gas. So, we need a different approach. 

Fortunately, there's another mean-field theory: Debye-Hückel theory.
...
where the factor of $1/2$ arises to account for the double counting of pair interaction energies. 
...
$$
(-q^2-\kappa_D^2)V(q)=-\frac{\rho_{ext}(q)}{D\epsilon_0}
$$

$$
V(q)=\frac{\rho_{ext}(q)}{D\epsilon_0} \frac{1}{q^2+\kappa_D^2}
$$
...
$$
U=\frac12 \int V \rho=\frac12 \int (\langle V \rangle+\delta V) (\langle \rho \rangle+\delta \rho)=\text{const.}+\frac12 \int \left(\langle V \rangle \delta \rho+ \delta V \langle \rho \rangle\right)  + O(\delta \rho \delta V)
$$

where const.$=\frac12 \int \langle V \rangle \langle \delta r \rangle$ is a term that is the same for every charge micro-state, and can therefore be ignored in the exponent of the Boltzmann factor.
...
$$
\frac12 \int \left(\langle V \rangle \delta \rho+ \delta V \langle \rho \rangle\right)  \to \int \langle V \rangle \delta \rho
$$
...
$$ 
\int  \delta V \langle \rho \rangle =-D\epsilon_0\int \delta V \nabla^2 \langle V \rangle =-D\epsilon_0\int \langle V \rangle \nabla^2 \delta V =\int \langle V \rangle \delta \rho\;,
$$

after (in total) four integrations by parts.
...
- Even if complementary charges get close, interaction potentials barely exceeds $k_BT$ (technically, because thermal fluctuations let charges fluctuate on a scale comparable to $\lambda_D$) $\Rightarrow$ need several electrostatic bonds.
